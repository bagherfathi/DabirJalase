...
- **Scaffold status:** `desktop-app/` and `python_services/` now include placeholder classes/modules, READMEs, and minimal build/test hooks so the team can begin implementing the MVP slice and wiring the CI templates described above. The Python services expose deterministic REST endpoints for transcription, diarization, summarization, and TTS (plus a lightweight session orchestrator that stitches the stubs together) to keep smoke tests runnable before model downloads are introduced, and can be launched locally with `python -m python_services` to mirror production-style config overrides.
