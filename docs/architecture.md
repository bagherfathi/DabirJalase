...
- **Repository layout (initial scaffold):**
  ...
  - **Scaffold status:** `desktop-app/` and `python_services/` now include placeholder classes/modules, READMEs, and minimal build/test hooks so the team can begin implementing the MVP slice and wiring the CI templates described above. The Python services expose deterministic REST endpoints for transcription, diarization, summarization, TTS, and a simple VAD span detector (plus a lightweight session orchestrator that stitches the stubs together with speaker-label prompts for "who is this?" moments, a raw-audio buffer endpoint (`/sessions/{id}/audio`) so capture clients can stage PCM chunks ahead of diarization, a buffer processor (`/sessions/{id}/process_buffer`) that VADs/diarizes staged PCM with transcript hints (and can clear the buffer once stitched), a VAD-driven `/sessions/{id}/ingest` hook that appends diarized segments from speech spans using transcript hints, and an export API that returns a meeting manifest with metadata + labels + summary, now with optional disk persistence for stored exports, a retention sweeper endpoint, and privacy/DSR helpers to redact speakers or delete sessions + stored manifests, with a restore hook to rehydrate sessions from stored manifests) to keep smoke tests runnable before model downloads are introduced. Sessions also expose a `/sessions/{id}/search` keyword endpoint (and matching client helper) to power "find in meeting" flows against diarized text + speaker labels. Launch locally with `python -m python_services` to mirror production-style config overrides or via the lean Dockerfile (`docker build -f python_services/Dockerfile -t meeting-assistant-services .` → `docker run -p 8000:8000 meeting-assistant-services`) when Python toolchains aren’t available. Request-level tracing (`x-request-id`), optional rate limiting, browser-friendly CORS headers, an opt-in `PY_SERVICES_API_KEY` guard, and a `/metrics` counter snapshot keep the scaffold deployable behind tunnels/gateways while the full authN/Z stack is wired, and deterministic offline harnesses are available via `python -m python_services.scripts.demo_session --persist ./data/exports` (demo run) or `python -m python_services.ops.smoke` (API smoke) to validate stitched flows and stored manifests without external services.
